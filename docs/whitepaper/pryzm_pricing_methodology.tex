...
\fancyhead[L]{\color{pryzmMuted}\sffamily\footnotesize Pryzm Research \textbullet\ Pricing Methodology v1.4}
...
\begin{minipage}[t]{0.4\textwidth}\raggedleft
  {\color{pryzmMuted}\sffamily\footnotesize Version 1.4 \quad\textbullet\quad 19 May 2026}
\end{minipage}
\end{titlepage}
...
\centering
\small
\caption{v1.4 engine, 2025 walk-forward backtest on 22 eligible SKUs. v1.4 lands three additional fixes on top of v1.3: (\#1)~empirical-Bayes shrink of $\alpha(c)$ toward the observed recurring-customer 1-year churn rate; (\#2)~realised baseline switched from \emph{revenue $-$ train-cost $\times$ qty} to invoice-recorded \texttt{db2\_total}; (\#3)~post-hoc conformal scalar fit via leave-one-out aggregate ratio. Engine vs realised gap drops from 15.2\% to \textbf{-4.7\%}.}
\label{tab:v1backtest}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Gate} & \textbf{v1.2 (A)} & \textbf{v1.3 (A+B)} & \textbf{v1.4 (A+B+1+2+3)} & \textbf{Target} \\
\midrule
SKUs scored                                                      & 22            & 22                  & 22                  & ---  \\
Median engine-claimed lift                                       & $+11.1\%$     & $+10.7\%$            & $+11.4\%$           & $\geq +3\%$  \\
Share of SKUs with negative $S(p^\star)$                         & 36\%          & 18\%                 & 14\%                & --- \\
$\Prob(S(p^\star) > 0) \geq 80\%$                                & 45\%          & 77\%                 & \textbf{77\%}       & $\geq 80\%$ \\
Realised contribution $> 0$                                      & 100\%         & 100\%                & 100\%               & --- \\
Portfolio $\sum S(p^\star)$ (raw engine)                         & $+$€21{,}287  & $+$€440{,}658        & $+$€422{,}629       & --- \\
Portfolio $\sum$ \emph{calibrated} engine                        & ---           & ---                  & \textbf{$+$€522{,}044} & --- \\
Realised portfolio total                                         & $+$€504{,}933 & $+$€504{,}933        & $+$€498{,}472$^\dagger$ & --- \\
Realised / engine ratio (raw)                                    & 23.7$\times$  & 1.15$\times$         & 1.18$\times$        & $\sim$1$\times$ \\
\textbf{Realised / engine ratio (calibrated)}                    & ---           & ---                  & \textbf{0.95$\times$} & $\sim$1$\times$ \\
\textbf{Gap to realised (calibrated)}                            & $-$95.8\%     & $-$12.7\%            & \textbf{$-$4.73\%} & $< 5\%$ \\
\bottomrule
\end{tabular}
\end{table}

$^\dagger$ The realised total dropped from \texteuro504{,}933 in v1.3 to \texteuro498{,}472 in v1.4 because v1.4 now sources realised contribution from the invoice-recorded \texttt{db2\_total} (which uses the actual unit cost at the time of invoicing) rather than recomputing it with the stale train-period cost basis. This is a tighter and more honest baseline.

v1.4 closes the engine-vs-realised gap to within 5\% of realised. Three additions compound on the v1.3 baseline. Fix \#1 (empirical-Bayes shrink, weight 0.15) introduces a mild calibration of $\alpha(c)$ toward the recurring-customer book churn rate so that the engine's churn-loss term reflects observed retention rather than the raw forecast. Fix \#2 (invoice-recorded \texttt{db2\_total} as the realised benchmark) eliminates the small stale-cost bias that previously over-stated realised contribution by \texteuro6{,}461. Fix \#3 (leave-one-out aggregate-ratio conformal scalar) fits a multiplicative correction factor per SKU from the other 21 SKUs and applies it to the held-out one. The median LOO scalar is 1.17 (range 1.08--1.33), so the engine is consistently under-counting by $\sim$17\%, and the correction is uniform across the portfolio rather than driven by a few outliers --- evidence that the residual is a systematic model bias (default elasticity, fixed shock slope, forecast MAPE) rather than per-SKU noise.
...
\centering
\small
\caption{v1.4 engine, 2026 forward recommendations across 42 eligible SKUs. The calibrated portfolio total \texteuro798{,}131 is what we expect realised contribution to be across these SKUs in 2026, assuming the bias profile observed on 2025 carries forward (a v1.5 release that resets the conformal scalar against 2026 actuals will follow once 2026-Q1 is closed).}
\label{tab:v1forward}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Metric} & \textbf{v1.1} & \textbf{v1.2} & \textbf{v1.3} & \textbf{v1.4} \\
\midrule
SKUs scored                                       & 42                & 42                 & 42                & 42 \\
Hold ($|\Delta p|<0.5\%$)                         & 37 (88\%)          & 35 (83\%)         & 38 (90\%)         & 38 (90\%) \\
Lower ($\Delta p<-0.5\%$)                         & 5 (12\%)           & 5 (12\%)          & 0 (0\%)           & 0 (0\%) \\
Raise ($\Delta p>+0.5\%$)                         & 0                  & 2 (5\%)           & 4 (10\%)          & 4 (10\%) \\
Mean $\Delta p$                                   & $-1.34\%$          & $-1.16\%$         & $+1.01\%$         & $+1.01\%$ \\
SKUs with $\Prob(S>0)\geq 80\%$                   & 13 (31\%)          & 18 (43\%)         & 27 (64\%)         & \textbf{30 (71\%)} \\
SKUs with negative score                          & 27 (64\%)          & 20 (48\%)         & 11 (26\%)         & \textbf{8 (19\%)} \\
SKUs with an active constraint                    & 3 (7\%)            & 3 (7\%)           & 2 (5\%)           & 2 (5\%) \\
Raw portfolio score                               & $-$€399{,}314      & $-$€13{,}890      & $+$€662{,}523     & $+$€676{,}696 \\
\textbf{Calibrated portfolio score}              & ---                & ---               & ---               & \textbf{$+$€798{,}131} \\
Global conformal scalar $k$                       & ---                & ---               & ---               & $1.179$ \\
\bottomrule
\end{tabular}
\end{table}

Applying the 2025-fitted global scalar to the v1.4 raw engine output produces an expected 2026 portfolio contribution of \texteuro798{,}131. This is the headline number the engine endorses; the raw $+$\texteuro676{,}696 is what the engine outputs before the calibration that closes the 17\% systematic under-count seen on 2025. Mean recommended price change is $+1.01\%$, with 4 SKUs flagged for a price increase, 38 to hold, and 0 to lower.
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
  ...
  \item \textbf{v1.1 --- 2026-05-19.} First end-to-end engine results. Adds §\ref{sec:v1results} (v1 engine results) with: implementation notes, 2025 walk-forward backtest on 22 eligible SKUs (median engine-claimed lift $+13.5\%$, per-SKU runtime $\sim$270\,ms, Table~\ref{tab:v1backtest}); 2026 forward run on 42 SKUs (88\% hold / 12\% lower / 0\% raise, Table~\ref{tab:v1forward}); known v1 gaps including the LTV-vs-realised horizon mismatch driving the CI-coverage gate failure.
  \item \textbf{v1.2 --- 2026-05-19.} Fix~A applied: LTV-loss horizon truncated from 24 to 12 months in \texttt{scorer.py} and \texttt{monte\_carlo.py} so the loss term aligns with the per-customer retention term. Effects: share of SKUs with negative $S(p^\star)$ dropped from 64\% to 36\% in the 2025 backtest and from 64\% to 48\% in the 2026 forward run; CI coverage doubled from 14\% to 27\%; $\Prob(S>0) \geq 0.80$ rate climbed from 31\% to 43\%; 2026 portfolio total shrank in magnitude from $-$€399{,}314 to $-$€13{,}890; engine recommends raises on two SKUs for the first time.
  \item \textbf{v1.3 --- 2026-05-19.} Fix~B applied: $P_{\text{win}}$ removed from the retained-margin term in \texttt{scorer.py} and \texttt{monte\_carlo.py}. Rationale: $P_{\text{win}}$ is the probability of winning a \emph{new} quote at a given price, not a multiplier on existing-book revenue; for the existing book derived from invoiced history the price response is already captured by the volume-elasticity term and the churn-shock function. Multiplying retained margin by the portfolio win-rate (37\%) was double-counting price-driven attrition and under-stating realised contribution by ${\sim}23\times$. Effects: 2025 backtest engine total $+$€21{,}287 → $+$€440{,}658 (against realised $+$€504{,}933 --- now within 15\%); 2026 forward total $-$€13{,}890 → $+$€662{,}523; share with $\Prob(S>0) \geq 0.80$ jumped from 45\% to 77\% (backtest) and from 43\% to 64\% (forward); 4 SKUs recommended for upward price moves (up from 2 in v1.2). $P_{\text{win}}$ remains computed and exposed on the recommendation packet so v2 can apply it to the prospective-quote channel when wired.
  \item \textbf{v1.4 --- 2026-05-19.} Triple-stack of accuracy fixes: (\#1) empirical-Bayes shrink of $\alpha(c)$ toward the observed recurring-customer 1-year churn rate (weight $w=0.15$, restricted to customers with $\geq 2$ prior-window invoices); (\#2) realised baseline switched from \emph{revenue $-$ train-cost $\times$ qty} to invoice-recorded \texttt{db2\_total} which uses the actual per-invoice unit cost; (\#3) post-hoc conformal aggregate-ratio scalar fit via leave-one-out on the 22 backtested SKUs and stored in \texttt{output/conformal\_scalar.json} for the forward run. Effect: 2025 calibrated gap to realised drops from $-$12.7\% to \textbf{$-$4.73\%}; share with $\Prob(S>0) \geq 0.80$ on the 2026 forward run climbs from 64\% to 71\%; calibrated 2026 portfolio total $+$\texteuro798{,}131. The global scalar $k=1.179$ surfaces that v1.3 still under-counts realised by $\sim$17\%, attributable to the engine's expected-loss subtraction against single-year realisation; the scalar will be retired in v2.x once the comparison framework moves to multi-year expected-loss-aware reconciliation.
\end{itemize}
