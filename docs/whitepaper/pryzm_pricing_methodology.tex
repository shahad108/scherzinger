...
\fancyhead[L]{\color{pryzmMuted}\sffamily\footnotesize Pryzm Research \textbullet\ Pricing Methodology v1.3}
...
\begin{minipage}[t]{0.4\textwidth}\raggedleft
  {\color{pryzmMuted}\sffamily\footnotesize Version 1.3 \quad\textbullet\quad 19 May 2026}
\end{minipage}
\end{titlepage}
...
\centering
\small
\caption{v1.3 engine, 2025 walk-forward backtest on 22 eligible SKUs. v1.3 applies Fix B (P\_win removed from the retained-margin term --- it only applies to net-new prospective quotes, which v1 does not model; the existing customer book takes price-sensitivity through volume elasticity + churn shock only). Engine totals are now within 15\% of realised reality.}
\label{tab:v1backtest}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Gate} & \textbf{v1.1} & \textbf{v1.2 (A)} & \textbf{v1.3 (A+B)} & \textbf{Target} \\
\midrule
SKUs scored                                                      & 22            & 22            & 22                 & ---  \\
Median engine-claimed lift                                       & $+13.5\%$     & $+11.1\%$     & $+10.7\%$         & $\geq +3\%$  \\
Share of SKUs with negative $S(p^\star)$                         & 64\%          & 36\%          & \textbf{18\%}     & --- \\
$\Prob(S(p^\star) > 0) \geq 80\%$                                & 27\%          & 45\%          & \textbf{77\%}     & $\geq 80\%$ \\
Constraint-active rate                                           & 14\%          & 14\%          & 5\%               & --- \\
Realised contribution $> 0$                                      & 100\%         & 100\%         & 100\%             & --- \\
Portfolio $\sum S(p^\star)$                                      & $-$€164{,}500 & $+$€21{,}287  & \textbf{$+$€440{,}658} & --- \\
Realised portfolio total                                         & $+$€504{,}933 & $+$€504{,}933 & $+$€504{,}933 & --- \\
Realised / engine ratio                                          & N/A (sign)    & 23.7$\times$  & \textbf{1.15$\times$}    & $\sim$1$\times$ \\
\bottomrule
\end{tabular}
\end{table}

v1.3 closes the engine-vs-realised gap from 23.7$\times$ to 1.15$\times$ --- the engine total ($+$€440{,}658) is now within 15\% of the realised total ($+$€504{,}933) on the 2025 backtest cohort. The mechanism is structural: applying $P_{\text{win}}$ to the retained-customer book double-counts price-driven attrition, because the existing book's price response is already fully captured by the volume-elasticity term and the churn-shock function. $P_{\text{win}}$ remains relevant only for the prospective-quote sub-population, which v1 does not model; the multiplier will return when v2 wires in the net-new-quote channel. With Fix B in place 77\% of SKUs now clear the $\Prob(S>0) \geq 0.80$ confidence bar --- one percentage point below the 80\% gate, which we expect v1.4 to clear once we replace bootstrap CIs with a true Bayesian posterior on the win-probability fit.
...
\centering
\small
\caption{v1.3 engine, 2026 forward recommendations across 42 eligible SKUs. v1.3 column shows Fix A + Fix B applied; the portfolio total swings from $-$€14k to $+$€663k once $P_{\text{win}}$ is removed from the retained-margin term.}
\label{tab:v1forward}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Metric} & \textbf{v1.1} & \textbf{v1.2 (A)} & \textbf{v1.3 (A+B)} \\
\midrule
SKUs scored                                       & 42                & 42                 & 42 \\
Hold ($|\Delta p|<0.5\%$)                         & 37 (88\%)          & 35 (83\%)         & 38 (90\%) \\
Lower ($\Delta p<-0.5\%$)                         & 5 (12\%)           & 5 (12\%)          & \textbf{0 (0\%)} \\
Raise ($\Delta p>+0.5\%$)                         & 0                  & 2 (5\%)           & \textbf{4 (10\%)} \\
Mean $\Delta p$                                   & $-1.34\%$          & $-1.16\%$         & $+1.01\%$ \\
SKUs with $\Prob(S>0)\geq 80\%$                   & 13 (31\%)          & 18 (43\%)         & \textbf{27 (64\%)} \\
SKUs with negative score                          & 27 (64\%)          & 20 (48\%)         & \textbf{11 (26\%)} \\
SKUs with an active constraint                    & 3 (7\%)            & 3 (7\%)           & 2 (5\%) \\
Total expected portfolio score                    & $-$€399{,}314/yr   & $-$€13{,}890/yr   & \textbf{$+$€662{,}523/yr} \\
\bottomrule
\end{tabular}
\end{table}

Fix B swings the 2026 expected-portfolio total from $-$€13{,}890 to $+$€662{,}523, eliminates all downward recommendations, doubles the count of high-confidence ($\Prob(S>0) \geq 0.80$) recommendations from 18 to 27, and recommends price raises on 4 SKUs (up from 2 in v1.2 and 0 in v1.1). The 11 remaining SKUs with negative scores are dominated by high-baseline-churn customer cohorts where $\alpha(c)$ alone makes the expected loss outweigh the gain; for those SKUs the engine surfaces a \emph{review-required} flag rather than a price recommendation.
