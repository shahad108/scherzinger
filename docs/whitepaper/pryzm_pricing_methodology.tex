...
\fancyhead[L]{\color{pryzmMuted}\sffamily\footnotesize Pryzm Research \textbullet\ Pricing Methodology v1.2}
...
\begin{minipage}[t]{0.4\textwidth}\raggedleft
  {\color{pryzmMuted}\sffamily\footnotesize Version 1.2 \quad\textbullet\quad 19 May 2026}
\end{minipage}
\end{titlepage}
...
\centering
\small
\caption{v1.2 engine, 2025 walk-forward backtest on 22 eligible SKUs. v1.2 fixes the horizon-mismatch bug from v1.1: the LTV-loss term in $S(p)$ is now truncated to 12 months to match the per-customer retention horizon. CI coverage and $\Prob(S>0) \geq 80\%$ both improved markedly without changing the directional recommendations.}
\label{tab:v1backtest}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Gate} & \textbf{v1.1} & \textbf{v1.2 (fix A)} & \textbf{Target} \\
\midrule
SKUs scored                                          & 22       & 22                  & --- \\
Median engine-claimed lift (p$^\star$ vs p$_{\text{actual}}$) & $+13.5\%$ & $+11.1\%$  & $\geq +3\%$  \\
Per-SKU runtime (incl. 400-draw MC)                  & $\sim 270$\,ms  & $\sim 720$\,ms & $<500$\,ms \\
$\Prob(S(p^\star) > 0) \geq 80\%$                    & 27\%     & \textbf{45\%}       & --- (diagnostic) \\
Constraint-active rate                               & 14\%     & 14\%                & --- (diagnostic) \\
Share of SKUs with realised contribution $> 0$       & 100\%    & 100\%               & --- (diagnostic) \\
Share of SKUs with negative $S(p^\star)$             & \textbf{64\%} & \textbf{36\%}   & --- (diagnostic) \\
Portfolio total $\sum S(p^\star)$                    & $-$€164{,}500 & $+$€21{,}287  & --- (diagnostic) \\
90\% MC-band coverage of realised contribution       & 14\%     & \textbf{27\%}       & $\geq 80\%$ (still open) \\
\bottomrule
\end{tabular}
\end{table}

v1.2 cuts the share of SKUs returning a negative $S(p^\star)$ nearly in half (64\% → 36\%) and flips the portfolio total from $-$€164{,}500 to $+$€21{,}287 by aligning the LTV-loss horizon to the per-customer retention horizon. Both quantities are now on a 12-month basis. The CI-coverage gate is still open at 27\% --- the next contributor is the bootstrap-vs-Bayesian gap in the win-probability posterior and a default elasticity that under-states price-sensitivity for SKUs with thin price variation; both are queued for v1.3. The 45\% rate of $\Prob(S > 0) \geq 0.80$ shows that the engine is now confidently positive on nearly half of the eligible book, up from one in four.
...
\centering
\small
\caption{v1.2 engine, 2026 forward recommendations across 42 eligible SKUs. v1.2 results in column 2; v1.1 (with the 24-month LTV horizon bug) shown for reference.}
\label{tab:v1forward}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Metric} & \textbf{v1.1} & \textbf{v1.2 (fix A)} \\
\midrule
SKUs scored                                       & 42                & 42 \\
Hold ($|\Delta p|<0.5\%$)                         & 37 (88\%)          & 35 (83\%) \\
Lower ($\Delta p<-0.5\%$)                         & 5 (12\%)           & 5 (12\%) \\
Raise ($\Delta p>+0.5\%$)                         & 0                  & \textbf{2 (5\%)} \\
Mean $\Delta p$                                   & $-1.34\%$          & $-1.16\%$ \\
Median $\Delta p$                                 & $0.00\%$           & $0.00\%$ \\
SKUs with $\Prob(S>0)\geq 80\%$                   & 13 (31\%)          & \textbf{18 (43\%)} \\
SKUs with negative score                          & 27 (64\%)          & \textbf{20 (48\%)} \\
SKUs with an active constraint                    & 3 (7\%)            & 3 (7\%) \\
SKUs with locked win-prob model (low quote data)  & 1 (2\%)            & 1 (2\%) \\
Total expected portfolio score                    & $-$€399{,}314/yr   & \textbf{$-$€13{,}890/yr} \\
\bottomrule
\end{tabular}
\end{table}

Truncating the LTV horizon to 12 months shrinks the total expected portfolio score from $-$€399{,}314 to $-$€13{,}890 --- a 28$\times$ reduction in the absolute magnitude of the headline number --- and enables the engine to recommend price increases on two SKUs for the first time. The remaining $\sim$48\% of SKUs with negative score are not measurement artefacts but legitimate engine output: the customer-level churn risk multiplied by retained-contribution LTV exceeds the win-weighted retained margin, so the engine is doing exactly what it should and warning that holding (or lowering) the price is the right call.
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
  ...
  \item \textbf{v1.2 --- 2026-05-19.} Fix~A applied: LTV-loss horizon truncated from 24 to 12 months in \texttt{scorer.py} and \texttt{monte\_carlo.py} so the loss term aligns with the per-customer retention term. Effects: share of SKUs with negative $S(p^\star)$ dropped from 64\% to 36\% in the 2025 backtest and from 64\% to 48\% in the 2026 forward run; CI coverage doubled from 14\% to 27\%; $\Prob(S>0) \geq 0.80$ rate climbed from 31\% to 43\%; 2026 portfolio total shrank in magnitude from $-$€399{,}314 to $-$€13{,}890; engine recommends raises on two SKUs for the first time. Remaining negatives are legitimate engine output, not bugs.
\end{itemize}
