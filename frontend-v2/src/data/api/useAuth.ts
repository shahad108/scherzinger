...
import mockMe from '@/data/mocks/me.json';
  ...
  return useMutation({
    mutationFn: async (vars: { email: string; password: string }) => {
      const me = await postJson<MeUser>('/auth/login', vars, {
        mockResolve: () => {
          if (vars.email !== 'frank@scherzinger.de' || vars.password !== 'frank-demo-2026') {
            throw new Error('Ungueltige Demo-Anmeldedaten');
          }
          return structuredClone(mockMe) as MeUser;
        },
      });
      return me;
    },
