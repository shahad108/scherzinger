      ...
      {isOpen && detail && (
        ...
        <div className="flex-shrink-0 px-6 py-4" style={{ background: '#ffffff', borderBottom: '1px solid #f8fafc' }}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-lg font-bold text-slate-800 mt-1">{detail.commodity_group.replace(/_/g, ' ')} — Commodity Group Overview</h3>
              <p className="text-sm text-slate-500">{detail.skuCount} Articles · {detail.atRiskCount} at risk · {detail.belowFloorCount} below floor</p>
            </div>
        ...
        <motion.div variants={slideOverSectionVariants} initial="hidden" animate="visible" className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <motion.div variants={slideOverItemVariants} className="grid grid-cols-4 gap-3">
            {[
              { label: 'Revenue', value: formatEUR(detail.totalRevenue), accent: 'linear-gradient(to right, #0393da, #c1e8ff)' },
              { label: 'Avg Margin', value: `${(detail.avgMargin * 100).toFixed(1)}%`, accent: detail.avgMargin < MARGIN_FLOOR ? 'linear-gradient(to right, #ef4444, #fca5a5)' : 'linear-gradient(to right, #22c55e, #86efac)' },
              { label: 'At Risk Articles', value: `${detail.atRiskCount} / ${detail.skuCount}`, accent: 'linear-gradient(to right, #f97316, #fdba74)' },
              { label: 'Recovery', value: formatEUR(detail.totalRecovery), sub: `${detail.pricingActions} price actions`, accent: 'linear-gradient(to right, #22c55e, #86efac)' },
            ].map((kpi) => (
          ...
          <motion.div variants={slideOverItemVariants} className="rounded-xl p-4" style={{ background: '#ffffff', boxShadow: '0 8px 32px rgba(26,26,46,0.04)' }}>
            <h4 className="text-sm font-bold text-slate-700 mb-3">Revenue by Year</h4>
            <div className="h-36">
          ...
          <motion.div variants={slideOverItemVariants} className="rounded-xl p-4" style={{ background: '#ffffff', boxShadow: '0 8px 32px rgba(26,26,46,0.04)' }}>
            <h4 className="text-sm font-bold text-slate-700 mb-3">Top Articles by Revenue</h4>
          ...
          {detail.skuBreakdown.filter(s => s.isAtRisk).length > 0 && (
            <motion.div variants={slideOverItemVariants} className="bg-red-50/30 rounded-lg border border-red-200 p-4">
              <h4 className="text-sm font-bold text-red-700 mb-3 flex items-center gap-2">
                <AlertTriangle size={14} /> Problem Articles ({detail.skuBreakdown.filter(s => s.isAtRisk).length})
              </h4>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="text-slate-400 uppercase text-[9px] font-bold border-b border-red-200">
                      <th className="py-2 pr-3">Article ID</th>
          ...
          {detail.forecast && (
            <motion.div variants={slideOverItemVariants} className="rounded-xl p-4" style={{ background: '#ffffff', boxShadow: '0 8px 32px rgba(26,26,46,0.04)' }}>
              <h4 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                <TrendingUp size={14} className="text-[#0393da]" /> Margin Forecast
              </h4>
          ...
          {detail.governance && (
            <motion.div variants={slideOverItemVariants} className="rounded-xl p-4" style={{ background: '#ffffff', boxShadow: '0 8px 32px rgba(26,26,46,0.04)' }}>
              <h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <ShieldCheck size={14} className="text-green-500" /> Governance Rules
              </h4>
